...
const Tools = () => {
  ...
  return (
    <motion.div
      ...
      <motion.div
        className='absolute  -translate-x-1/2 top-[40vh]  shadow-[0px_0px_300px_50px_#0ea5e9]'
        animate={{
          x: [30, -20, 20, -30],
          y: [30, -20, 20, -30],
          rotate: [0, 90],
      ...
      <div className='flex gap-2 justify-center items-center flex-wrap'>
        {categories.map((c, i) => (
          <div
            ...
            id={c}
            key={'i' + i}
            className={`text-xs inline-flex items-center font-bold leading-sm uppercase px-3 py-1 ring-0  rounded-full cursorClick ${
              query == c
      ...
      <div className='grid w-full md:grid-cols-2 lg:grid-cols-3 gap-4'>
        {toolsFiltered.map((t, i) => (
          <motion.a
            key={i}
            href={t.url}
            className={`cursorClick ${!isLoaded && 'invisible'}`}
            target={'_blank'}
            ...
            <div className='overflow-hidden  aspect-video rounded-xl relative group'>
              <div className='rounded-xl z-[5] opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out absolute from-black/80 to-transparent bg-gradient-to-t inset-x-0 -bottom-2 pt-30 text-white flex items-end'>
                <div className='w-full flex justify-between items-center p-4 space-y-3 text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-4 pb-10 transform transition duration-300 ease-in-out'>
                  ...
                  <div
                    key={i}
                    className='text-xs inline-flex items-center font-bold leading-sm uppercase px-3 py-1 bg-gradient-to-r  from-sky-500 to-sky-700 ring-0 text-white rounded-full '>
                    {t.category}
                  </div>
                </div>
              </div>
              <img
                ...
                src={t.image}
                onLoad={() => {
                  i == toolsFiltered.length - 1 && setIsLoaded(true);
                }}
              />
            </div>
          </motion.a>
